...
export async function buildDigest(siteUrl: string) {
  ...
  const html = `<div style="max-width:640px;margin:0 auto;padding:20px;background:#fff">
    ...
    ${due.length ? `<div style="font-family:-apple-system,Segoe UI,sans-serif;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#B7791A;font-weight:600;margin:12px 0 4px">Follow up today (Working list)</div><table width="100%" cellspacing="0" cellpadding="0">${due.map(row).join('')}</table>` : ''}
