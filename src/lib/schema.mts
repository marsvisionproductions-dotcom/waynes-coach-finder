...
export function normalize(r: RawListing): NormalizedListing {
  ...
  const locText = r.location || (r.description || '').match(/(?:[Ll]ocated in|[Ll]ocation:?|[Nn]ear)\s+([A-Z][A-Za-z .]+?,\s*(?:[A-Z]{2}|[A-Z][a-z]+(?: [A-Z][a-z]+)?))\b/)?.[1] || (r.description || '').match(/\b([A-Z][A-Za-z .]{2,25},\s*[A-Z]{2})\b(?:\s+\d{5})?/)?.[1] || (r.title || '').match(/\b([A-Z][A-Za-z .]{2,25},\s*[A-Z]{2})\b/)?.[1] || (r.description || '').match(/\b([A-Z][a-z]+(?: [A-Z][a-z]+)?)\s+([A-Z]{2})\s+\d{5}\b/)?.slice(1, 3).join(', ');
  ...
  const n: NormalizedListing = {
    ...
    seller_type, seller_name: r.seller_name || undefined,
...
export function isRelevant(n: NormalizedListing, minPrice = 0): boolean {
  ...
  if (/\b(wanted|looking for|iso\b|parts only|for parts|toy hauler|trailer|fifth wheel|5th wheel|tour bus|shell only|seats|bus conversion project)\b/i.test(n.title_raw || '')) return false;
  return true;
}
