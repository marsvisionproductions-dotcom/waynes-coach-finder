...
export function normalize(r: RawListing): NormalizedListing {
  ...
  const locText = r.location || (r.description || '').match(/(?:located in|location:?|near)\s+([A-Z][A-Za-z .]+,\s*(?:[A-Z]{2}|[A-Z][a-z]+(?: [A-Z][a-z]+)?))\b/i)?.[1] || (r.description || '').match(/\b([A-Z][A-Za-z .]{2,25},\s*[A-Z]{2})\b(?:\s+\d{5})?/)?.[1] || (r.title || '').match(/\b([A-Z][A-Za-z .]{2,25},\s*[A-Z]{2})\b/)?.[1] || (r.description || '').match(/\b([A-Z][a-z]+(?: [A-Z][a-z]+)?)\s+([A-Z]{2})\s+\d{5}\b/)?.slice(1, 3).join(', ');
