...
export interface FeedDef { source: string; url: string; seller_type?: 'private' | 'unknown'; }

export const FEEDS: FeedDef[] = [
  { source: 'prevostrvforsale', url: 'https://www.prevostrvforsale.com/feed/rss/', seller_type: 'private' },
  { source: 'newellgurus',      url: 'https://www.newellgurus.com/syndication.php?fid=27&limit=30', seller_type: 'private' },
  { source: 'foreforums',       url: 'https://www.foreforums.com/index.php?action=.xml;type=rss2;limit=30', seller_type: 'private' },
];

interface FeedItem { title: string; link: string; description: string; pubDate?: string; guid?: string }
...
export function parseFeed(xml: string): FeedItem[] {
  ...
  for (const m of xml.matchAll(/<(item|entry)\b[\s\S]*?<\/\1>/gi)) {
    ...
    items.push({ title: pick(b, 'title'), link, description: pick(b, 'description') || pick(b, 'content:encoded') || pick(b, 'content') || pick(b, 'summary'), pubDate: pick(b, 'pubDate') || pick(b, 'published') || pick(b, 'updated') || pick(b, 'dc:date') || undefined, guid: pick(b, 'guid') || pick(b, 'id') || undefined });
  }
  return items;
}
...
export function feedItemToRaw(def: FeedDef, it: FeedItem): RawListing | null {
  ...
  return {
    ...
    posted_at: it.pubDate ? new Date(it.pubDate) : null, seller_type: def.seller_type,
    location: text.match(/\b(?:located in|location:?)\s*([A-Za-z .]+,?\s*[A-Z]{2})\b/i)?.[1],
...
export async function collectFeeds(defs: FeedDef[] = FEEDS): Promise<{ items: RawListing[]; errors: string[] }> {
  ...
  for (const def of defs) {
    try {
      ...
      for (const it of parseFeed(r.text)) { const raw = feedItemToRaw(def, it); if (raw) items.push(raw); }
